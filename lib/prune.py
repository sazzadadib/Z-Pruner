...
def z_pruner(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if args.prune_method == "wanda":
                W_metric = torch.abs(W) * torch.sqrt(wrapped_layers[name].scaler_row.reshape((1,-1)))
            elif args.prune_method == "normax":
                  def compute_importance_scores_tuned(W):
                       
                        # Row-wise L2 normalization with improved stability
                        norm_row = torch.norm(W, p=2, dim=1, keepdim=True)
                        W_norm_row = W / (norm_row)  

                        # Column-wise L2 normalization with improved stability
                        norm_col = torch.norm(W, p=2, dim=0, keepdim=True)
                        W_norm_col = W / (norm_col)  

                        # Global statistics (using the normalized matrices)
                        ...
                        # Compute normalized deviations with improved stability
                        ...
                        importance_col = torch.abs(diff_col**2) * torch.abs(diff_col)
                         ...
                         # Add this frequency-aware scaling factor
                        sparsity = (W.abs() < W.abs().mean() * 0.1).float().mean()
                        alpha = 0.7 * (1 - 0.3 * sparsity)  # Dynamically adjust alpha based on sparsity
                  ...
                  #W_metric = compute_importance_scores_tuned(W) * activation
                  W_metric = compute_importance_scores_tuned(W) * (activation2)


            print("::::::::::::::::::::::::::::::::::::W Metric:::::::::::::::::::::",W_metric)
